...
export function PassthroughTab({
        ...
        async function fetchStatus(): Promise<void> {
            ...
            if (isLinux()) {
                ...
                if (existsSync(PASSTHROUGH_LOG_ERR)) {
                    const tail = await execa("tail", ["-n", "50", PASSTHROUGH_LOG_ERR], {
                        reject: false,
                    });
                    if (!cancelled) {
    ...
    if (isMacOS()) {
        const { daemonInstalled, legacyAgentInstalled, configFound, config, autosshPid } = macState;
            ...
            <Box flexDirection="column" gap={1}>
                <Box flexDirection="column">
                    ...
                    <Box gap={1}>
                        ...
                        {configFound && config ? (
                            <Text>
                                {config.tunnelUser}@{config.vpsAddress}:{config.vpsPort} → tunnel :
                                {config.tunnelPort}
                            </Text>
                        ) : (
                    ...
                    <Box marginTop={1} flexDirection="column">
                        ...
                        {!daemonInstalled && (
                            <Text dimColor>
                                Run `syncreeper-passthrough setup` to install the on-start config.
                            </Text>
                        )}
                    </Box>
                </Box>
